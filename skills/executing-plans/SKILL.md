...
<!-- copilot-cli adaptation: Native task operations (TaskGet/TaskUpdate/TaskList,
the ready/next-task query, dependency checks) map to the `sql` tool on the `todos` +
`todo_deps` tables — see using-superpowers/references/task-management.md. Mark a task
`in_progress` before starting and `done` on completion. There is no
EnterPlanMode/ExitPlanMode on Copilot CLI. Use `ask_user` for AskUserQuestion. -->
