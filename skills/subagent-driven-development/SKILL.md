...
<!-- copilot-cli adaptation: Dispatch subagents with the `task` tool
(agent_type "general-purpose" for implementation/review, "explore" for research);
use `read_agent`/`list_agents` for status. Native task reads/updates map to `sql` on
the `todos`/`todo_deps` tables — see using-superpowers/references/task-management.md.
Pass each subagent ONLY the task's metadata you SELECT from `todos.description`;
never let it inherit your session context. -->
