...
<!-- copilot-cli adaptation: "Dispatch a subagent" = call the `task` tool
(agent_type "general-purpose" for work, "explore" for research). Run truly
independent agents by issuing multiple `task` calls in a SINGLE turn; use
`read_agent`/`list_agents` to collect results. There is no Claude `subagent_type`
parameter — pick the Copilot `agent_type` instead. -->
