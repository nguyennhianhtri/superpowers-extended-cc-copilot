...
<!-- copilot-cli adaptation: Use `ask_user` (one question at a time, with `choices`)
wherever this skill says AskUserQuestion. Write the captured answers back into the
task by UPDATEing the `json:metadata` fence inside that row's `todos.description` via
the `sql` tool — see using-superpowers/references/task-management.md. -->
