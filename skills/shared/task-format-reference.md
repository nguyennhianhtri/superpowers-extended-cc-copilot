...
> **Copilot CLI:** there is no `TaskCreate` tool. Create the task with the `sql` tool
> by `INSERT`ing into the `todos` table and putting this entire structured
> description (including the `json:metadata` fence) into the `description` column.
> Record dependencies in `todo_deps`. The metadata schema below is unchanged. See
> `using-superpowers/references/task-management.md` for the exact SQL.
