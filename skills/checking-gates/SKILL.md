...
<!-- copilot-cli adaptation: TaskGet/TaskUpdate map to `sql` SELECT/UPDATE on the
`todos` table; parse the `json:metadata` fence out of `todos.description` — see
using-superpowers/references/task-management.md. Because Copilot CLI hooks do not
fire at runtime yet, this skill is enforced by DISCIPLINE: run it before marking any
task `done` whose description contains `"userGate": true`. There is no /gate-check
slash command surface — invoke this skill by name. -->
