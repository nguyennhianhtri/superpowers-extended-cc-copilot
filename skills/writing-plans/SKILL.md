...
<!-- copilot-cli adaptation: This skill creates native tasks. On Copilot CLI there
are no TaskCreate/TaskGet/TaskUpdate/TaskList tools — use the `sql` tool on the
`todos` + `todo_deps` tables instead. Record task dependencies in `todo_deps` to get
the no-front-running guarantee. Full mapping (with the metadata-in-description
pattern and the "ready query"): using-superpowers/references/task-management.md.
There is no EnterPlanMode/ExitPlanMode — write the plan to a file and pause for
review. Use `ask_user` wherever this skill says AskUserQuestion. -->
...
## Gate enforcement note (only when user-gate tasks were tagged)

> **Copilot CLI:** there is no `.claude/settings.json` to scan and no close-time
> hook fires. Skip the settings-file detection below. Instead, when the plan
> contains any `userGate: true` task, show the short heads-up unconditionally, and
> rely on the **discipline** wired by this port: before marking any such task
> `done`, the `checking-gates` skill MUST run. (See the
> `superpowers-extended-cc-copilot` README → "Feature parity".)

If the plan contains any task with `userGate: true`, show this short heads-up before
the Execution Handoff (substitute {N} and the task numbers):

> Heads up — I tagged {N} task(s) as user-gate (Tasks #X, #Y, …). The plan runs
> end-to-end as-is. Before I close any of these I will run the `checking-gates`
> skill to re-validate its acceptance criteria with captured evidence.

Suppress entirely if no user-gate tasks were tagged. Do NOT turn this into an
`ask_user` question.

<details><summary>Claude Code original (not applicable on Copilot CLI)</summary>
